		...
		<body>
			...
			<nav id="first">
				<div id="first_menu">
				    ...
				    <a  href="/ClearLoginServlet">退出登录</a>
				    <!-- 相对于端口号的相对路径 -->
					<a  href="/jsp/alter.jsp">账号管理</a>
					<a  href="/jsp/my_page.jsp">个人主页</a>
					
					<a href="/chatServlet?method=my_chat_list">豆邮</a>
					
					<a href="/FriendListServlet?method=blacklist_list">黑名单</a>
					<a href="/FriendListServlet?method=attention_list">我的关注</a>
					<a href="/FriendListServlet?method=friend_list">我的好友</a>
					<!-- 先跳转servlet 查询第一页的所有人信息 -->
					<a href="/EveryoneListServlet?method=everyone_list">所有人</a>
					<!-- 编辑文章 -->
					<a href="/jsp/article_edit.jsp" target="_blank">写文章</a>
					<a href="/ArticleListServlet?method=article_list">所有文章</a>
					<a href="/ArticleListServlet?method=my_article_list" >我的文章</a>
					<a href="/ArticleListServlet?method=my_collection_list">我的收藏</a>
				</div>
			</nav>
			<!-- 第二个导航栏 -->
			<header id="second">
					<nav  id="second_menu">
						<!-- logo部分 -->
							<div class="logo">
							 	<img alt="豆瓣logo" src="/image/豆瓣首页logo.jpg" width=175px height=58px >
							</div>
						<!-- 导航栏部分_可选择部分 -->
							<div class="navbar">
								<ul>
									<li><a href="#">首页</a></li>
									<li><a href="/jsp/my_page.jsp">个人主页</a></li>
									<li><a href="/ArticleListServlet?method=article_list">浏览发现</a></li>
								</ul>	
							</div>
							<!-- 搜索框部分 -->
							<div class="search">
									...
									<div class="item">
									        ...
									        <div class="content">
									        	<!--文章的标题 -->
											     <div class="title">
											     		...
											     		<!--文章跳转链接 -->
												        <a href="/ArticleShowServlet?method=article_show&article_id=${a.articleId}" target="_blank" style="color:black">
								        ...
								        <div class="bottom">
									        ...
									        <c:if test="${requestScope.starStatus == '已点赞文章' }"> 
											  <a href="/ArticleShowServlet?pre_method=article_star_cancel&method=article_show&article_id=${requestScope.article.articleId}" class="user_operation_btn" id="" >${requestScope.article.starNum}取消点赞</a>
											</c:if> 
									        <c:if test="${requestScope.starStatus == '未点赞文章' }"> 
											  <a href="/ArticleShowServlet?pre_method=article_star&method=article_show&article_id=${requestScope.article.articleId}" class="user_operation_btn" id="" >${requestScope.article.starNum}点赞</a>
											</c:if> 
									         <!-- 文章收藏 -->
									        <c:if test="${requestScope.collectionStatus == '已收藏文章' }"> 
											  <a href="/ArticleShowServlet?pre_method=article_collection_cancel&method=article_show&article_id=${requestScope.article.articleId}" class="user_operation_btn" id="" >${requestScope.article.collectionNum}取消收藏</a>
											</c:if> 
									        <c:if test="${requestScope.collectionStatus == '未收藏文章' }"> 
											  <a href="/ArticleShowServlet?pre_method=article_collection&method=article_show&article_id=${requestScope.article.articleId}" class="user_operation_btn" id="" >${requestScope.article.collectionNum}收藏</a>
											</c:if> 
									        <!-- 文章转发-->
									        <c:if test="${requestScope.shareStatus == '已转发文章' }"> 
											  <a href="/ArticleShowServlet?pre_method=article_share_cancel&method=article_show&article_id=${requestScope.article.articleId}" class="user_operation_btn" id="" >${requestScope.article.shareNum}取消转发</a>
											</c:if> 
									        <c:if test="${requestScope.shareStatus == '未转发文章' }"> 
											  <a href="/ArticleShowServlet?pre_method=article_share&method=article_show&article_id=${requestScope.article.articleId}" class="user_operation_btn" id="" >${requestScope.article.shareNum}转发</a>
											</c:if> 
