...
#submit_btn{
					...
					<div id="main_content-left-centre">
				        ...
				        <!-- 豆邮的每一项 --> 
				         ...
				         <div class="chat-form-div" >
							...
							<%--对话--隐藏表单 --%>	
					        <form action="/chatServlet?pre_method=chat_content&method=chat_show" id="onSubmit" name="chat_form" method="post" >
					        	<!-- 发表对话的文本框 -->
					          <div class="item">
					              <textarea name="chat-place" required  id="chat-textarea" rows="4" cols="64"  onKeyUp="checkMaxInput(this)" onKeyDown="checkMaxInput(this)"></textarea>
					              <br>
					          </div>
	...
	function formSubmit(){
		...
		var y = document.getElementById("chat-textarea");
...
/***********************************************/
	...
	function toAjax(c){
		...
		xmlHttpRequest.open("post","/chatServlet",true);
		//设置post方式的头信息--文件上传application/x-www-form-urlencoded  multipart/form-data --文件上传不能加这个标头设置
		xmlHttpRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		xmlHttpRequest.send("method=chat_show_details&to_user_id=" + ${requestScope.toUserId} + "&currentPage=" + c);
	}
			...
			//设置服务器返回值的类型是String格式--JSON字符串---JSON.parse将字符串转化为对象
			 obj = JSON.parse(xmlHttpRequest.responseText);  
				 for (var i = 0; i<obj.objects.length;i++ ) {
						...
						'						<div class=\"chat-item\" id=\"'+obj.objects[i].chatId+'\" style=\"    display: flex;box-shadow: 0 0 black; \" >'+
						'						    <!-- 对话人的头像 -->'+
						...
						'						    </div>'+
						'						    <div class="content report-chat" style="float: left;height: auto;width: 590px;margin: 11px 0px;">'+
						'						        <div class="author">'+
						'						        	<!-- 对话时间 -->'+
						...
						'						'+
						'						        <!-- 对话的内容-->'+
						'						        <p id=\"chat-msg\" style="margin: 10px 2px;height: auto;    line-height: 1.6;font-size: 13px;    width: 575px;word-wrap: break-word;overflow: hidden;">'+obj.objects[i].chatMsg+
						'						</p>'+
						'						    </div>'+
						'<input type=\"hidden\"   value = \"'+obj.objects[i].chatId+'\">'+
						'						</div>'+
						'';
							    $("#chat-code").prepend($chat);
	         }
